#include <SPI.h>
#include <Ethernet.h>
#include "Dhcp.h"  // uses DHCP code from http://www.mcqn.com/weblog/dhcp_and_dns_arduino
#include <TextFinder.h>
#include "chartbeat_settings.h"

byte mac[] = { 
  0xDE, 0xAD, 0xBE, 0xEF, 0xFE, 0xED };
byte server[] = { 184,73,162,97 }; // api.chartbeat.com
char* apiKey = APIKEY;
char* channelIDs[]={
  "fotbollskanalen.se","nyheterna.se","tv4.se","tv4play.se"};  
long waitTill[4] = {
  0, 0, 0, 0};
long stateclicks[4] = {
  -1, -1, -1, -1};
...
long getStateclicks(int channel) {
  ...
  boolean ok = false;
  if (client.connect()) {
    client.print("GET /quickstats?host=");
    client.print(channelIDs[channel]);
    client.print("&apikey=");
    client.print(apiKey);
    client.println(" HTTP/1.0");  
    client.println("Host: api.chartbeat.com");
    client.println();
  } 
  else {
